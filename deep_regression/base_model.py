...
class BaseModel(object):
    def __init__(self, config):
        """
        回归的基类，提供了各种属性和训练，验证，测试的方法
        :param config: 模型的配置参数
        """
        self.config = config
        self.inputs = tf.placeholder(tf.float32, [None, None], name="inputs")  # 数据输入
    ...
    def cal_loss(self):
        ...
        with tf.name_scope("loss"):
            ...
            loss = tf.sqrt(tf.reduce_mean(tf.square(self.labels - self.logits)))
            # loss = losses/tf.cast(tf.shape(self.labels)[0],dtype=tf.float32)
            return loss
